...
create table if not exists public.pn_notes (
  ...
  constraint pn_notes_section_fk foreign key (project_id, section_id)
    references public.pn_sections(project_id, id) on delete set null,
...
create table if not exists public.pn_deletion_log (
  id uuid primary key default gen_random_uuid(),
...
alter table public.pn_deletion_log enable row level security;
...
create policy pn_projects_authenticated_insert on public.pn_projects for insert to authenticated with check (created_by = auth.uid());
create policy pn_projects_authenticated_delete on public.pn_projects for delete to authenticated using (true);
create policy pn_sections_authenticated_insert on public.pn_sections for insert to authenticated with check (created_by = auth.uid());
create policy pn_sections_authenticated_delete on public.pn_sections for delete to authenticated using (true);
create policy pn_notes_authenticated_insert on public.pn_notes for insert to authenticated with check (created_by = auth.uid());
...
with check (exists (select 1 from public.pn_notes n where n.id = note_id and n.project_id = project_id));
...
with check (
  ...
  and exists (select 1 from public.pn_notes n where n.id = note_id and n.project_id = project_id)
);

create policy pn_activity_authenticated_select on public.pn_activity for select to authenticated using (true);
create policy pn_deletion_log_authenticated_select on public.pn_deletion_log for select to authenticated using (true);

create or replace function public.pn_set_note_status(target_note_id uuid, next_status text, status_comment text default null)
...
begin
  ...
  if updated_note.id is null then raise exception 'PN note not found'; end if;
  ...
  values (updated_note.project_id, updated_note.id, 'status_changed', auth.uid(), jsonb_build_object('status', next_status));
...
create policy pn_files_public_select on storage.objects for select to anon, authenticated using (bucket_id = 'pn-files');
...
create policy pn_files_authenticated_delete on storage.objects for delete to authenticated using (bucket_id = 'pn-files');

-- Preserve existing Arabesque data without modifying the legacy module.
...
from public.projects p where p.slug = 'arabesque'
...
from public.project_note_sections s join public.pn_projects p on p.id = s.project_id
on conflict (id) do nothing;

insert into public.pn_notes(id, project_id, section_id, title, description, status, position, created_by, created_at, updated_at, completed_at)
select n.id, n.project_id, n.section_id, n.title, n.description,
...
from public.project_notes n join public.pn_projects p on p.id = n.project_id
on conflict (id) do nothing;
...
from public.project_note_comments c join public.pn_notes n on n.id = c.note_id
on conflict (id) do nothing;

insert into public.pn_attachments(id, project_id, note_id, bucket_id, object_path, file_name, mime_type, file_size, created_at)
select a.id, a.project_id, a.note_id, a.bucket_id, a.object_path, a.file_name, a.mime_type, a.file_size, a.created_at
from public.project_note_attachments a join public.pn_notes n on n.id = a.note_id
on conflict (id) do nothing;

create or replace function public.pn_log_project_delete()
returns trigger language plpgsql security definer set search_path = public as $$
begin
  insert into public.pn_deletion_log(entity_type, entity_id, project_id, title, actor_id)
  values ('project', old.id, old.id, old.name, auth.uid());
  return old;
end;
$$;

create or replace function public.pn_log_section_delete()
returns trigger language plpgsql security definer set search_path = public as $$
begin
  insert into public.pn_deletion_log(entity_type, entity_id, project_id, title, actor_id)
  values ('section', old.id, old.project_id, old.title, auth.uid());
  return old;
end;
$$;

create or replace function public.pn_log_note_delete()
returns trigger language plpgsql security definer set search_path = public as $$
begin
  insert into public.pn_deletion_log(entity_type, entity_id, project_id, title, actor_id)
  values ('note', old.id, old.project_id, old.title, auth.uid());
  return old;
end;
$$;

revoke all on function public.pn_log_project_delete() from public, anon, authenticated;
revoke all on function public.pn_log_section_delete() from public, anon, authenticated;
revoke all on function public.pn_log_note_delete() from public, anon, authenticated;

create trigger pn_projects_delete_audit before delete on public.pn_projects for each row execute function public.pn_log_project_delete();
create trigger pn_sections_delete_audit before delete on public.pn_sections for each row execute function public.pn_log_section_delete();
create trigger pn_notes_delete_audit before delete on public.pn_notes for each row execute function public.pn_log_note_delete();
